...
impl RecordPlugin for PostGatePlugin {
    ...
    async fn insert(
        ...
        if let Some(post) = post_uri {
            let existing = client
                .query_opt(
                    "SELECT uri FROM post_gate WHERE postUri = $1",
                    &[&post],
                )
                .await
        ...
        // Update post to set hasPostGate = true
        if let Some(post) = post_uri {
            client
                .execute(
    ...
    async fn delete(&self, pool: &Pool, uri: &str) -> Result<(), IndexerError> {
        ...
        let row = client
            .query_opt("SELECT postUri FROM post_gate WHERE uri = $1", &[&uri])
            .await
            .map_err(|e| IndexerError::Database(e.into()))?;
        ...
        // Update post to set hasPostGate = false
        if let Some(post) = post_uri {
            client
                .execute(
