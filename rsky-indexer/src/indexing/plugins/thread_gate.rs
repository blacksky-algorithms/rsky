...
impl RecordPlugin for ThreadGatePlugin {
    ...
    async fn insert(&self, pool: &Pool, uri: &str, cid: &str, record: &JsonValue, timestamp: &str) -> Result<(), IndexerError> {
        ...
        client.execute(
            "INSERT INTO thread_gate (uri, cid, post_uri, created_at, indexed_at) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (uri) DO NOTHING",
    ...
    async fn delete(&self, pool: &Pool, uri: &str) -> Result<(), IndexerError> {
        let client = pool.get().await?;
        client.execute("DELETE FROM thread_gate WHERE uri = $1", &[&uri]).await.map_err(|e| IndexerError::Database(e.into()))?;
        Ok(())
    }
}
