...
impl RecordPlugin for ListItemPlugin {
    ...
    async fn insert(&self, pool: &Pool, uri: &str, cid: &str, record: &JsonValue, timestamp: &str) -> Result<(), IndexerError> {
        ...
        let list = record.get("list").and_then(|v| v.as_str());
        ...
        client.execute(
            "INSERT INTO list_item (uri, cid, creator, subject_did, list_uri, created_at, indexed_at) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (uri) DO NOTHING",
            &[&uri, &cid, creator, &subject, &list, &created_at, &indexed_at],
    ...
    async fn delete(&self, pool: &Pool, uri: &str) -> Result<(), IndexerError> {
        let client = pool.get().await?;
        client.execute("DELETE FROM list_item WHERE uri = $1", &[&uri]).await.map_err(|e| IndexerError::Database(e.into()))?;
        Ok(())
    }
}
