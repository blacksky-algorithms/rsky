...
#[async_trait]
impl RecordPlugin for StatusPlugin {
    ...
    async fn insert(&self, pool: &Pool, uri: &str, cid: &str, _record: &JsonValue, _timestamp: &str) -> Result<(), IndexerError> {
        ...
        client.execute("INSERT INTO status (uri, cid) VALUES ($1, $2) ON CONFLICT (uri) DO NOTHING", &[&uri, &cid]).await.map_err(|e| IndexerError::Database(e.into()))?;
        Ok(())
    }
    ...
    async fn delete(&self, pool: &Pool, uri: &str) -> Result<(), IndexerError> {
        let client = pool.get().await?;
        client.execute("DELETE FROM status WHERE uri = $1", &[&uri]).await.map_err(|e| IndexerError::Database(e.into()))?;
        Ok(())
    }
}
