...
pub fn get_did_key_from_multibase(key: VerificationMaterial) -> Result<Option<String>> {
    ...
    let did_key = match key.r#type.as_str() {
        "EcdsaSecp256r1VerificationKey2019" => Some(format_did_key(P256_JWT_ALG.to_string(), key_bytes)?),
        "EcdsaSecp256k1VerificationKey2019" => Some(format_did_key(SECP256K1_JWT_ALG.to_string(), key_bytes)?),
        "Multikey" => {
