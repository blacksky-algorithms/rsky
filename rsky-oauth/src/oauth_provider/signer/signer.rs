use crate::jwk::{
    Audience, JwkError, JwtConfirmation, JwtPayload, Key, Keyset, SignedJwt, VerifyResult,
};
...
impl Signer {
    ...
    pub async fn verify(
        ...
        verify_options: Option<Validation>,
        ...
        let verify_options = match verify_options {
            ...
            Some(verify_options) => {
                ...
                verify_options.set_issuer(&vec![self.issuer.clone()]);
                Some(verify_options)
            }
        };
...
mod tests {
    ...
    async fn test_verify() {
        ...
        let result = signer.verify(token, Some(validation)).await;
    }
