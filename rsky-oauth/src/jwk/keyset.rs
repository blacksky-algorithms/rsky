...
use biscuit::jwk::PublicKeyUse;
use rocket::form::validate::Contains;
use std::collections::HashSet;

pub struct Keyset {
    keys: Vec<Box<dyn Key>>,
    preferred_signing_algorithms: Vec<SignatureAlgorithm>,
}
...
pub struct KeySearch {
    ...
    pub alg: Option<Vec<SignatureAlgorithm>>,
}
...
impl Keyset {
    ...
    //
    // pub fn sign_algorithms(&self) -> Vec<SignatureAlgorithm> {
    ...
    async fn find_key(
    ...
    ) -> Result<(&Box<dyn Key>, SignatureAlgorithm), JwkError> {
        ...
        let candidates: Vec<(&Box<dyn Key>, Vec<SignatureAlgorithm>)> = matching_keys
            .into_iter()
            .map(|matching_key| (matching_key, matching_key.algorithms()))
            .collect();
...
#[cfg(test)]
mod tests {
    // use crate::jwk::{Audience, JwtPayload, Key, Keyset, SignedJwt, VerifyResult};
    ...
    //             x509_fingerprint: Some("NEMyMEFCMzUwMTE1QTNBOUFDMEQ1ODczRjk5NzBGQzY4QTk1Q0ZEOQ".to_string()),
    //         },
    ...
    //     let result = keyset.verify_jwt(token, Some(validation)).await.unwrap();
