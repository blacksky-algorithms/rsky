...
pub enum JarError {
    ...
    #[error("signing key decoding error")]
    SigningKeyDecoding,
}
...
mod tests {
    ...
    //         // Test HS384
    //         let hs384_jar =
    //             OAuthAuthorizationRequestJar::new(claims.clone(), Some(SignatureAlgorithm::HS384), Some(key))
    //                 .unwrap();
    //         assert!(hs384_jar.is_signed());
    //
    //         // Test HS512
    //         let hs512_jar =
    //             OAuthAuthorizationRequestJar::new(claims.clone(), Some(SignatureAlgorithm::HS512), Some(key))
    //                 .unwrap();
