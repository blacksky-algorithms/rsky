...
impl OAuthAuthorizationDetail {
    ...
    /// Set the locations.
    pub fn with_locations(mut self, locations: Vec<String>) -> Result<Self, AuthorizationDetailError> {
        // Validate URLs
    ...
    pub fn with_additional_field(mut self, key: impl Into<String>, value: impl Into<serde_json::Value>) -> Self {
    ...
    pub fn from_json(json: &str) -> Result<Self, AuthorizationDetailError> {
        serde_json::from_str(json)
            .map_err(|_| AuthorizationDetailError::InvalidJson)
    }
    
    /// Convert to JSON.
    pub fn to_json(&self) -> Result<String, AuthorizationDetailError> {
        serde_json::to_string(self)
            .map_err(|_| AuthorizationDetailError::SerializationError)
    }
}
...
mod tests {
    ...
    fn test_from_json() {
        ...
        assert_eq!(detail.locations(), Some(&["https://example.com/accounts".to_string()][..]));
        assert_eq!(detail.actions(), Some(&["read".to_string(), "write".to_string()][..]));
        assert_eq!(detail.identifier(), Some("account123"));
